<?php
	...
	if(isset($_GET['delete']) && is_numeric($_GET['delete'])) {
		$id = $_GET['delete'];
		$mysqli->query("DELETE FROM `page_images` WHERE `id` = '$id' LIMIT 1");
		header('Location: /admin/page-images.php');
		exit;
	}
	
	// Upload new slide images
	$admin->uploadPageImg($mysqli);
	
	$slideQuery = $mysqli->query("SELECT * FROM `page_images`");
		...
		<div class="container">
			...
			<div style="float: right; width: 600px;">
				...
				<div class="uploadNewSlide">
					<h3>Upload New Page Image</h3>
					<form action="" method="post" enctype="multipart/form-data">
						<input type="file" name="newPageImg" />
						<input type="hidden" name="uploadPageImg" />
